...
class Stemresnet34(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(Stemresnet34, self).__init__()
...
class ALLSearch_resnet34(nn.Module):
    def __init__(self, input_channels,n_classes):
        ...
        for layer in range(self.layers):
            if layer == 0:
                ...
                self.layermodule.append(Stemresnet34(input_channels, nb_filter[0]))         
                ...
                for stage in range(self.layers-layer):
                    if stage == 0: #第一層 (64+64,64),(64*2+64,64)
                        self.layermodule.append(ConcatBlock((self.inifilter + self.inifilter) * np.power(self.multiplier,stage) * layer + self.inifilter * np.power(self.multiplier,stage+1) , self.inifilter * np.power(self.multiplier,stage) + self.inifilter))
                ...
                for stage in range(self.layers-layer-1): #ex layer1 有四個stage,只有三個downsampling
                    if stage == 0:
                        self.layermodule.append(ConcatBlock(self.inifilter * np.power(self.multiplier,stage) + self.inifilter,self.inifilter * np.power(self.multiplier,stage+1))) # for downsampling + maxpooling  
                    else :
    ...
    def forward(self, input):  
        ...
        return self.final(self.up(layer4[0]))      
...
if __name__ == '__main__':
    ...
    model = ALLSearch_resnet34(1,1).to(device)
