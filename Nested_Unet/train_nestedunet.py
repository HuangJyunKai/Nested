...
def train_model(model, criterion, optimizer, train_loader, scheduler, epoch, num_epochs):
    ...
    print("per_class_iou :",per_class_iou)
    print("mIOU :",mIOU)
    dirName = "./models/Line_nas_seed38_256_512/"
    if not os.path.exists(dirName):
       os.mkdir(dirName)
       print("Directory " , dirName ,  " Created ")  
    if num_epochs == 29:
        torch.save(model.state_dict(), dirName+'Line_NestedUNetROS_seed38_256_512_weights_epoch_%d.pth' % (epoch+1))
    return epoch_loss/step, overall_acc, per_class_acc, per_class_iou, mIOU
...
def train(args):
    ...
    step_size  = 50
    gamma      = 0.5
    model = NasUNet(3,nclass).to(device)
    ...
    for epoch in range(num_epochs):
        ...
        fp = open("Line_nas_seed38_256_512_epoch_%d.txt" % num_epochs, "a")
